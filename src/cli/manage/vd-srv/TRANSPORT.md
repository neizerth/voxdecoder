...
**gRPC transport (ADR 0007):** optional listener (`--grpc` / `[grpc]`, default bind `127.0.0.1:7702`). Disabled by default. Services: `PlanningService`, `ExecutionService`, `OperatorService` (includes **Health**), `EventService` (`WatchJob` stream). Observe RPCs are **typed** (`JobView`, `Event` oneof, `HealthResponse`) for codegen; Planning + `SubmitJob` stay `JsonBody`. See [proto/README.md](proto/README.md) and [ADR 0007](../../../../docs/adr/0007-runtime-api-transports.md).
