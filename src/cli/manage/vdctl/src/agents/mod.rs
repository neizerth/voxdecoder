...
pub struct Agent {
    ...
    pub name: String,
    pub installed: bool,
    /// MCP (or equivalent) integration already present for VoxDecoder.
    pub configured: bool,
...
impl AgentAdapter {
    ...
    /// Preferred MCP config file (first path; created on register if missing).
...
pub fn print_agents_human(agents: &[Agent]) {
    ...
    for a in agents {
        ...
        println!("{mark} {:<width$}{extra}", a.name, width = width);
    }
}
...
pub fn filter_installed_adapters(apps: Option<&[String]>) -> Result<Vec<AgentAdapter>, Error> {
    ...
    if let Some(apps) = apps {
        let wanted: std::collections::HashSet<_> =
            apps.iter().map(|s| s.to_ascii_lowercase()).collect();
