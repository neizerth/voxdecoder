...
| tests (CLI) | `src/cli/process/vd-assets/tests/{unit,e2e}/` | `./scripts/test.sh vd-assets` |

Shared crates: [`src/crates/`](../../../crates/). Change progress / platform `paths` there. Convert + dictionary stay in this crate (also linked from `vd-fix-*` as a lib for `load_dictionary`).
