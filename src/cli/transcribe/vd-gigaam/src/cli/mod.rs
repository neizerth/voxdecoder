...
Models are installed under the GigaAM cache by default (~/.cache/gigaam on Unix; override with --download-root / VD_GIGAAM_MODELS_DIR)."
)]
struct Root {
    #[command(subcommand)]
    command: RootCommand,
...
vd-gigaam -i meeting.ogg -m v3_e2e_ctc --dry-run\n  \
vd-gigaam run -i call.wav -o out.txt --format json --segments"
)]
pub struct RunCli {
    ...
    /// Checkpoint / converted-model directory (default: GigaAM cache)
...
then convert to SafeTensors when convert_ckpt.py (or VD_GIGAAM_CONVERT_SCRIPT) is available.\n\n\
