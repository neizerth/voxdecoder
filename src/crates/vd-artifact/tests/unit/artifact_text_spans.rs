...
use vd_artifact::{apply_to_text_spans, load_from_str, Artifact, VttBlock};
use vd_artifact::{ArtifactType, TextSpan};
...
fn vtt_preserves_cue_id_and_timing() {
    ...
    let Some(VttBlock::Cue { id, timing, text }) = body
        .blocks
        .iter()
        .find(|b| matches!(b, VttBlock::Cue { .. }))
    else {
        panic!("expected cue");
    };
