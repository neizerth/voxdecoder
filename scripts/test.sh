...
case "${1:-all}" in
  ...
  vd-fix-terms)
    ...
    ;;
  *)
    echo "usage: $0 [all|vd-gigaam|crates|vd-assets|vd-diarize|vd-meeting|vd-preprocess|vd-postprocess|vd-pipeline|vd-srv|vd-fix-casing|vd-fix-asr|vd-fix-terms] [cargo test args...]" >&2
    exit 2
    ;;
esac
