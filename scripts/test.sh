#!/usr/bin/env bash
set -euo pipefail

root="$(cd "$(dirname "$0")/.." && pwd)"
cd "$root"

run_all() {
  cargo test -p vd-gigaam
  cargo test -p vd-artifact -p vd-output -p vd-progress -p vd-input
  cargo test -p vd-assets
  cargo test -p vd-diarize
    cargo test -p vd-meeting
  cargo test -p vd-preprocess
  cargo test -p vd-postprocess
  cargo test -p vd-url
  cargo test -p vd-pipeline
  cargo test -p vd-srv
  cargo test -p vd-mcp
  cargo test -p vdctl
  cargo test -p vd-fix-casing
  cargo test -p vd-fix-asr
  cargo test -p vd-fix-disfluency
  cargo test -p vd-fix-terms
  cargo test -p vd-fix-layout
  cargo test -p vd-fix-overlap
}

case "${1:-all}" in
  all)
    run_all
    ;;
  vd-gigaam)
    cargo test -p vd-gigaam "${@:2}"
    ;;
  crates)
    cargo test -p vd-artifact -p vd-output -p vd-progress -p vd-input "${@:2}"
    ;;
  vd-input)
    cargo test -p vd-input "${@:2}"
    ;;
  vd-assets)
    cargo test -p vd-assets "${@:2}"
    ;;
  vd-diarize)
    cargo test -p vd-diarize "${@:2}"
    ;;
  vd-meeting)
    cargo test -p vd-meeting "${@:2}"
    ;;
  vd-preprocess)
    cargo test -p vd-preprocess "${@:2}"
    ;;
  vd-postprocess)
    cargo test -p vd-postprocess "${@:2}"
    ;;
  vd-url)
    cargo test -p vd-url "${@:2}"
    ;;
  vd-pipeline)
    cargo test -p vd-pipeline "${@:2}"
    ;;
  vd-srv)
    cargo test -p vd-srv "${@:2}"
    ;;
  vd-mcp)
    cargo test -p vd-mcp "${@:2}"
    ;;
  vdctl)
    cargo test -p vdctl "${@:2}"
    ;;
  vd-fix-casing)
    cargo test -p vd-fix-casing "${@:2}"
    ;;
  vd-fix-asr)
    cargo test -p vd-fix-asr "${@:2}"
    ;;
  vd-fix-disfluency)
    cargo test -p vd-fix-disfluency "${@:2}"
    ;;
  vd-fix-terms)
    cargo test -p vd-fix-terms "${@:2}"
    ;;
  vd-fix-layout)
    cargo test -p vd-fix-layout "${@:2}"
    ;;
  vd-fix-overlap)
    cargo test -p vd-fix-overlap "${@:2}"
    ;;
  *)
    echo "usage: $0 [all|vd-gigaam|crates|vd-input|vd-assets|vd-diarize|vd-meeting|vd-preprocess|vd-postprocess|vd-url|vd-pipeline|vd-srv|vd-mcp|vdctl|vd-fix-casing|vd-fix-asr|vd-fix-disfluency|vd-fix-terms|vd-fix-layout|vd-fix-overlap] [cargo test args...]" >&2
    exit 2
    ;;
esac
