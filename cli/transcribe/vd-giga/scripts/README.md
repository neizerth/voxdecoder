...
```bash
cd cli/transcribe/vd-giga/scripts
...
`vd-giga install MODEL` downloads the `.ckpt` from the GigaAM CDN, verifies MD5, then
runs this script automatically when `scripts/convert_ckpt.py` (or `VD_GIGA_CONVERT_SCRIPT`)
...
```bash
cargo run -p vd-giga -- run -i sample.wav -m v3_e2e_ctc \
  --download-root cli/transcribe/vd-giga/models --device cpu
```
