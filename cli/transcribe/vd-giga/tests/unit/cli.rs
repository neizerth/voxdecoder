...
use vd_giga::cli::{parse_args, CliError, Command as VdCommand, ProgressMode};
use vd_giga::config::resolve::{Device, OutputFormat};
use vd_giga::gigaam::catalog::resolve_model_name;

fn parse(args: &[&str]) -> Result<VdCommand, CliError> {
    let mut full = vec!["vd-giga"];
    full.extend(args);
    parse_args(full)
}
...
fn binary_unknown_flag_exits_2() {
    let bin = env!("CARGO_BIN_EXE_vd-giga");
