//! End-to-end binary tests against the `vd-giga` executable.
...
fn bin() -> assert_cmd::Command {
    assert_cmd::Command::new(cargo_bin!("vd-giga"))
}
...
fn with_isolation(cmd: &mut assert_cmd::Command, config: &Path, models: &Path) {
    cmd.env("VD_GIGA_CONFIG", config)
        .env("VD_GIGA_MODELS_DIR", models);
}
...
fn e2e_golden_ctc_transcription() {
    ...
    let status = Command::new(cargo_bin!("vd-giga"))
        .args([
            "run",
            "-i",
            wav.to_str().unwrap(),
...
fn e2e_ctc_converted_smoke_if_present() {
    ...
    let out = dir.path().join("out.txt");
    bin()
        .env("VD_GIGA_MODELS_DIR", models.parent().unwrap())
        .args([
            "run",
            "-i",
            wav.to_str().unwrap(),
