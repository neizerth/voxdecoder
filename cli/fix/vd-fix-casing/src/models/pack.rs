...
pub struct Lexicon {
    ...
    pub question_start: Vec<String>,
    pub soft_break: Vec<String>,
    #[serde(default)]
    pub question_particles: Vec<String>,
}
...
/// Packs are optional for the rules backend — `install` caches/overrides, not a hard requirement.
...
fn ru_lexicon() -> Lexicon {
    Lexicon {
        ...
        .map(str::to_string)
        .collect(),
        soft_break: vec!["а", "но", "или", "либо", "поэтому", "потом"]
            .into_iter()
            .map(str::to_string)
            .collect(),
        question_particles: vec!["ли".into()],
    }
}
...
fn en_lexicon() -> Lexicon {
    Lexicon {
        ...
        .collect(),
        question_start: vec![
            "who", "what", "where", "when", "why", "how", "is", "are", "do", "does", "did", "can",
            "could", "would", "will", "should", "have", "has",
        ]
        .into_iter()
        .map(str::to_string)
        .collect(),
        soft_break: vec!["but", "or", "so", "then", "because"]
            .into_iter()
            .map(str::to_string)
            .collect(),
        question_particles: vec![],
    }
}
