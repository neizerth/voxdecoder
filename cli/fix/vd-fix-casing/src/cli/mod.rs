...
#[command(
    name = "vd-fix-casing",
    version,
    about = "Local presentation fixer (punctuation, casing, whitespace)",
    long_about = "Rewrites only presentation. The input artifact type and structure are preserved.\n\n\
Shorthand: vd-fix-casing -i FILE  ≡  vd-fix-casing run -i FILE\n\n\
Language packs are optional for the built-in rules backend (vd-fix-casing install ru)."
)]
struct Root {
    #[command(subcommand)]
    command: RootCommand,
...
enum RootCommand {
    /// Fix presentation in a local text artifact
    Run(RunCli),
...
#[command(
    about = "Fix presentation in a local text artifact",
    after_help = "Examples:\n  \
vd-fix-casing install ru\n  \
vd-fix-casing run -i meeting.txt\n  \
vd-fix-casing -i meeting.txt --dry-run"
)]
pub struct RunCli {
